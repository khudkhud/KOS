...
def test_navdp_waits_then_timeouts_when_hpu_busy():
    osm = OSMStore()
    ...
    try:
        sup.send_goal("S-1", "navigation.navdp.predict_waypoint", {"goal": "kitchen"})
        raise AssertionError("expected hpu timeout")
    except RuntimeError as exc:
        assert str(exc).startswith("HPU_TIMEOUT")
    assert any(e.type == "HPU_WAIT_STARTED" for e in osm.event_log)
    assert any(e.type == "HPU_WAIT_TIMEOUT" for e in osm.event_log)
...
def test_base_contention_fails_immediately():
    osm = OSMStore()
    leases = LeaseManager(osm)
    leases.acquire(["base"], "S-owner")
    try:
        leases.acquire(["base"], "S-other")
        raise AssertionError("expected base busy")
    except RuntimeError as exc:
        assert str(exc).startswith("BASE_BUSY")


def test_hpu_wait_executes_when_released_within_soft_window():
    osm = OSMStore()
    waiter = Session(session_id="S-wait", owner="o", priority=1, capabilities=["NAV"], state=SessionState.EXECUTING)
    owner = Session(session_id="S-owner", owner="o", priority=1, capabilities=["NAV"], state=SessionState.EXECUTING)
    osm.apply_patch({"type": "session_upsert", "session": waiter})
    osm.apply_patch({"type": "session_upsert", "session": owner})

    leases = LeaseManager(osm)
    [lease] = leases.acquire(["hpu"], "S-owner")
    sup = ActionSupervisor(osm, DDSActionClient(InMemoryDDSBroker()), leases=leases)
    ...
    t.start()
    h = sup.send_goal("S-wait", "planning.robobrain.plan", {"intent": "wait"})
    t.join(timeout=1.0)

    assert h.session_id == "S-wait"
    assert any(e.type == "HPU_WAIT_ACQUIRED" for e in osm.event_log)

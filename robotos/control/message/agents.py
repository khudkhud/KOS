...
def build_default_agent_registry() -> AgentRegistry:
    ...
    reg.register(AgentDescriptor(agent_id="nav_service_agent", role="service", subscriptions=["REQ_NAV_PLAN"], publish_allow=["Decision", "Event"]))
    reg.register(AgentDescriptor(agent_id="task_execution_agent", role="task", subscriptions=["NAV_EXEC_DONE", "*"], publish_allow=["Request", "Proposal", "Event"]))
    return reg

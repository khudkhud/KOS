...
DEFAULT_RESOURCE_POLICIES: Dict[str, ResourceLeasePolicy] = {
    ...
    "hpu": ResourceLeasePolicy(ttl_ms=5_000, renew_interval_ms=1_500, timeout_mode="hard", on_expire="fail", lease_required=True, exclusive=True),
    # single mobile base(chassis), strict exclusive ownership and hard timeout
    "base": ResourceLeasePolicy(ttl_ms=2_000, renew_interval_ms=600, timeout_mode="hard", on_expire="fail", lease_required=True, exclusive=True),
    # camera is continuous 30fps service stream; tasks subscribe instead of owning it
    "camera": ResourceLeasePolicy(ttl_ms=0, renew_interval_ms=0, timeout_mode="soft", on_expire="retry", lease_required=False, exclusive=False),
}
...
class LeaseManager:
    ...
    def policy_for(self, resource: str) -> ResourceLeasePolicy:
        return self.resource_policies.get(
            resource,
            ResourceLeasePolicy(ttl_ms=5_000, renew_interval_ms=1_500, timeout_mode="hard", on_expire="fail", lease_required=True, exclusive=True),
        )
